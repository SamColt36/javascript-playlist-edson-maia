...
const alertCustom = () => {
  Swal.fire({
    ...
    footer:
      '<a href="https://medium.com/code-prestige/alertas-bonitos-responsivos-e-customizados-com-o-sweetalert2-8db930038137">Gostou do alert() diferenciado?! Confere o link.</a>',
  });
};
...
$btnCreate.click(async function (e) {
  const situationInputDate = Boolean($dataInput.val());
  const situationInputQuote = Boolean($quoteInput.val());
  const situationInputAuthorship = Boolean($authorshipInput.val());

  if (
    situationInputDate !== false &&
